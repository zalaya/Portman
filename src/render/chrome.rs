...
pub fn render_header(frame: &mut Frame, area: Rect, session: &Session) {
    ...
    let (exposed, critical) = session.items.iter().fold((0, 0), |(exposed, critical), usage| {
    ...
    spans.push(sep());
    spans.push(Span::styled(format!("sort: {}", session.sort.label()), Style::default().fg(theme::muted())));
    spans.push(sep());
...
pub fn render_search(frame: &mut Frame, area: Rect, session: &Session) {
    let text = Line::from(vec![
        ...
        Span::styled(session.filter.clone(), Style::default().fg(theme::primary())),
    ]);
