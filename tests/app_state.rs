...
fn killing_portman_itself_is_blocked() -> anyhow::Result<()> {
    ...
    session.items = vec![support::loopback_tcp(9999, support::own_pid(), "test-harness")];
    session.selected = Some(0);

    session.request_kill(KillSignal::Force);

    assert!(session.kill_target.is_none(), "should refuse to set up a kill target for portman's own process");
    assert_eq!(session.status.as_deref(), Some("That's portman itself — can't kill it from here"));
...
fn killing_another_process_asks_for_confirmation_first() -> anyhow::Result<()> {
    ...
    let target = session.kill_target.as_ref().expect("should stage a kill target for a process that isn't us");
    assert_eq!(target.pid, 424_242);
    assert_eq!(target.signal, KillSignal::Terminate);

    session.cancel_kill();
    assert!(session.kill_target.is_none(), "cancelling should clear the pending kill");
...
fn cycling_sort_visits_every_key_and_returns_to_port() -> anyhow::Result<()> {
    ...
    assert_eq!(session.sort, SortKey::Port, "five cycles should be a full loop back to the start");
...
fn opening_the_action_menu_on_a_tcp_port_offers_the_browser_action() -> anyhow::Result<()> {
    ...
    let menu = session.action_menu.as_ref().expect("selecting a row and opening the menu should populate it");
...
fn opening_the_action_menu_offers_copy_command_when_details_have_a_command_line() -> anyhow::Result<()> {
    ...
    session.items = vec![support::loopback_tcp(8080, u32::MAX, "dev-server")];
    session.selected = Some(0);
    session.details = Some(support::details_with_command(u32::MAX, vec!["node", "server.js"]));
...
fn selecting_a_running_process_populates_the_details_pane() -> anyhow::Result<()> {
    ...
    session.items = vec![support::loopback_tcp(3000, support::own_pid(), "test-harness")];
    session.selected = None;
    session.next();

    let details = session.details.as_ref().expect("selecting a real, running process should populate the details pane");
...
fn filtering_to_no_matches_clears_selection_and_details() -> anyhow::Result<()> {
    ...
    session.selected = None;
    session.next();
    assert!(session.details.is_some(), "sanity check: something should be selected before we filter it away");
